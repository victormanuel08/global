    ...
    <div class="flex gap-3 bg-gray-100 border" style="height: calc(150vh - 50px) !important">
      ...
      <div class="w-48 bg-gray-100 hidden md:block">
        ...
        <div v-for="(item, index) in menuItems" :key="item.path" class="my-4">
          <UButton class="rounded-full" @click="toggleChilds(index)" style="width: 200px; white-space: normal;" :to="item.path">
            <Icon :name="item.icon" style="color: white; font-size: 1.5em;" />
            {{ item.label }}
          </UButton>
          <div class="my-2" style="text-align: right;">
            <UButton style="width: 150px; white-space: normal; text-align: right;" v-for="subItem in item?.childs" :key="subItem.path" class="rounded-full" variant="soft" v-if="showChilds[index]" :to="subItem.path">
      ...
      <div class="flex-grow bg-white rounded-lg">
        ...
        <div class="flex justify-between p-2 bg-gray-100 rounded-full shadow m-4 hidden md:flex"> 
            <span>
        ...
        <div class="bg-gray-100 rounded-full shadow m-2 text-center block md:hidden">
          <span v-for="(item, index) in menuItems" :key="item.path" class="p-1" :style="{ marginBottom: index < menuItems.length - 1 ? '10rem' : '5' }">
            <UButton :to="item.path" class="rounded-full" @click="toggleChilds(index)">
              <Icon :name="item.icon" style="color: white; font-size: 1.5em;" />
            </UButton>
            <span class="p-1">
              <UButton v-for="subItem in item?.childs" :key="subItem.path" class="rounded-full" variant="soft" v-if="showChilds[index]" :to="subItem.path">
                <Icon :name="subItem.icon" style="color: blue; font-size: 1.5em;" />
              </UButton>
            </span>
          </span>
